...
class EdgeView extends View implements ItemListener, ActionListener {
	...
	public void fit() {
		...
		scale = (off_dimension.width - 40) / width;
	...
	public void update(Graphics g2) {
		...
		g2d.setRenderingHint(
				...
				BoardCAD.getInstance().isAntialiasing() ? RenderingHints.VALUE_ANTIALIAS_ON
						: RenderingHints.VALUE_ANTIALIAS_OFF);
		g2d.setRenderingHint(
				RenderingHints.KEY_TEXT_ANTIALIASING,
				BoardCAD.getInstance().isAntialiasing() ? RenderingHints.VALUE_TEXT_ANTIALIAS_ON
		...
		board_handler.draw_edge(g2d, off_dimension.width, off_dimension.height,
	...
	MouseMotionListener EdgeMouseMotion = new MouseMotionAdapter() {
		...
		public void mouseDragged(MouseEvent e) {
			if (e.isMetaDown()) {
				...
				if (is_marked) {
					...
					design_panel.set_coordinates(board_handler.get_x(),
							board_handler.get_y(), board_handler.get_z());
					design_panel.redraw();
				} else {
					dragged_x = e.getX();
					dragged_y = e.getY();
					if (dragged_y > clicked_y)
						dragged_y = clicked_y
								+ (int) (Math.abs(dragged_x - clicked_x)
										* (double) off_dimension.height / off_dimension.width);
					else
						dragged_y = clicked_y
								- (int) (Math.abs(dragged_x - clicked_x)
										* (double) off_dimension.height / off_dimension.width);
		...
		public void mouseMoved(MouseEvent e) {
			if (!is_marked)
				design_panel.set_coordinates(board_handler.get_x(),
						(-e.getY() + offset_y) / scale, (e.getX() - offset_x)
								/ scale);
		}
	};
			...
			if (e.isMetaDown()) {
				...
				if (is_dragged) {
					...
					if (is_marked) {
						...
						scale = (double) (off_dimension.width) / zoom_width
								* scale;
						offset_x = (int) ((double) (off_dimension.width)
								/ zoom_width * (offset_x - zoom_x));
						offset_y = (int) ((double) (off_dimension.width)
								/ zoom_width * (offset_y - zoom_y));
						repaint();
					}
				}
			}
...
class OutlineView extends View implements ItemListener, ActionListener {
	...
	public void fit() {
		...
		scale = (double) (off_dimension.width - 40)
				/ (double) board_handler.get_board_length();
		offset_x = 20;
		offset_y = off_dimension.height / 2;
		repaint();
	}
	...
	public void update(Graphics g2) {
		...
		board_handler.draw_outline(g2d, off_dimension.width,
				off_dimension.height, scale, offset_x, offset_y,
				t_under_item.getState(), bottom_cut_item.getState(),
				deck_cut_item.getState(), false);

		if (blank_item.getState())
			board_handler.draw_blank_outline(g2d, off_dimension.width,
					off_dimension.height, scale, offset_x, offset_y,
					t_under_item.getState());

		if (curvature_item.getState())
			board_handler.draw_outline_curvature(g2d, off_dimension.width,
					off_dimension.height, scale, offset_x, offset_y);
	...
	MouseMotionListener OutlineMouseMotion = new MouseMotionAdapter() {
		...
		public void mouseMoved(MouseEvent e) {
			if (!is_marked)
				design_panel.set_coordinates((e.getX() - offset_x) / scale,
						board_handler.get_y(), (e.getY() - offset_y) / scale);
		}
	};
...
class RockerView extends View implements ItemListener, ActionListener {
	...
	public void fit() {
		...
		scale = (double) (off_dimension.width - 40)
				/ (double) board_handler.get_board_length();
		offset_x = 20;
		offset_y = 2 * off_dimension.height / 3;
		repaint();
	}
	...
	public void update(Graphics g2) {
		...
		g2d.setRenderingHint(
				...
				BoardCAD.getInstance().isAntialiasing() ? RenderingHints.VALUE_TEXT_ANTIALIAS_ON
						: RenderingHints.VALUE_TEXT_ANTIALIAS_OFF);
		board_handler.draw_rocker(g2d, off_dimension.width,
				off_dimension.height, scale, offset_x, offset_y,
				vee_item.getState(), false, false, false);

		if (blank_item.getState())
			board_handler.draw_blank_rocker(g2d, off_dimension.width,
					off_dimension.height, scale, offset_x, offset_y,
					vee_item.getState());

		if (curvature_item.getState())
			board_handler.draw_rocker_curvature(g2d, off_dimension.width,
					off_dimension.height, scale, offset_x, offset_y);
	...
	MouseMotionListener RockerMouseMotion = new MouseMotionAdapter() {
		...
		public void mouseMoved(MouseEvent e) {
			if (!is_marked)
				design_panel.set_coordinates((e.getX() - offset_x) / scale,
						(offset_y - e.getY()) / scale, board_handler.get_z());
		}
	};
...
class ThreeDView2 extends View implements ItemListener, ActionListener {
	...
	public ThreeDView2(BoardHandler bh, DesignPanel dp) {
		...
		theta = 30;
		double[][] m1 = {
				{ Math.cos(-theta * Math.PI / 180.0), 0.0,
						-Math.sin(-theta * Math.PI / 180.0) },
				{ 0.0, 1.0, 0.0 },
				{ Math.sin(-theta * Math.PI / 180.0), 0.0,
						Math.cos(-theta * Math.PI / 180.0) } };
		zeta = 45;
		double[][] m2 = {
				{ 1.0, 0.0, 0.0 },
				{ 0.0, Math.cos(-zeta * Math.PI / 180.0),
						-Math.sin(-zeta * Math.PI / 180.0) },
				{ 0.0, Math.sin(-zeta * Math.PI / 180.0),
						Math.cos(-zeta * Math.PI / 180.0) } };

		rotation_matrix = MathUtils.cross_product(m1, m2);
	...
	public void fit() {
		...
		scale = 0.9 * (off_dimension.width - 40)
				/ board_handler.get_board_length();
		offset_x = 100;
	...
	public void update(Graphics g2) {
		...
		board_handler.draw_outline3D(g2d, off_dimension.width,
				off_dimension.height, scale, offset_x, offset_y,
				t_under_item.getState(), bottom_cut_item.getState(),
				deck_cut_item.getState(), rotation_matrix);
	...
	MouseMotionListener OutlineMouseMotion = new MouseMotionAdapter() {
		...
		public void mouseDragged(MouseEvent e) {
			if (e.isMetaDown()) {
				...
				double[][] m1 = {
						{ Math.cos(-theta2 * Math.PI / 180.0), 0.0,
								-Math.sin(-theta2 * Math.PI / 180.0) },
						{ 0.0, 1.0, 0.0 },
						{ Math.sin(-theta2 * Math.PI / 180.0), 0.0,
								Math.cos(-theta2 * Math.PI / 180.0) } };
				double zeta2 = zeta + dragged_y - clicked_y;
				double[][] m2 = {
						{ 1.0, 0.0, 0.0 },
						{ 0.0, Math.cos(-zeta2 * Math.PI / 180.0),
								-Math.sin(-zeta2 * Math.PI / 180.0) },
						{ 0.0, Math.sin(-zeta2 * Math.PI / 180.0),
								Math.cos(-zeta2 * Math.PI / 180.0) } };
				...
				if (is_marked) {
					...
					double[][] m = MathUtils.invert(rotation_matrix);

					// board_handler.set_x((e.getX()-offset_x)/scale);
					double myy = (rotation_matrix[1][0] * p.x
							+ rotation_matrix[1][1] * p.y + rotation_matrix[1][2]
							* p.z);

					board_handler.set_x(m[0][0] * (e.getX() - offset_x) / scale
							+ m[0][1] * myy + m[0][2] * (e.getY() - offset_y)
							/ scale);// + m[0][2]*100);
					board_handler.set_y(m[1][0] * (e.getX() - offset_x) / scale
							+ m[1][1] * myy + m[1][2] * (e.getY() - offset_y)
							/ scale);// + m[1][2]*100);
					board_handler.set_z(m[2][0] * (e.getX() - offset_x) / scale
							+ m[2][1] * myy + m[2][2] * (e.getY() - offset_y)
							/ scale);// + m[2][2]*100);
		...
		public void mousePressed(MouseEvent e) {
			...
			if (e.isMetaDown()) {
				...
				if (is_marked) {
					...
					design_panel.set_coordinates(board_handler.get_x(),
							board_handler.get_y(), board_handler.get_z());
					p = new NurbsPoint(board_handler.get_x(),
							board_handler.get_y(), board_handler.get_z());
